...
// This is a temporary solution to avoid the dotenv dependency
const ENV = {
...
function processTemplate(filePath) {
    try {
        ...
        content = content.replace(
            ...
            `<meta name="supabase-anon-key" content="${ENV.SUPABASE_ANON_KEY}">`
        );
        
        // Create embedded Supabase client to avoid CORS issues
        const embedSupabaseScript = `
    ...
    <script>
        ...
        // Insert the script at the end of the head
        content = content.replace(
            '</head>',
            `${embedSupabaseScript}\n</head>`
        );
