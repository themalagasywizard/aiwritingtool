  ...
  return {
    auth: {
      getUser: async () => {
        ...
        return { 
          data: { 
            user: mockAuthState ? { email: mockAuthState.email, id: mockAuthState.id } : null 
          },
          error: null
        };
      },
      ...
      getSession: async () => {
        ...
        return { 
          data: { 
            session: mockAuthState ? { user: { email: mockAuthState.email, id: mockAuthState.id } } : null 
          },
          error: null
        };
      },
      ...
      signInWithPassword: async ({ email, password }) => {
        ...
        const userId = mockAuthState?.id || 'mock_' + Math.random().toString(36).substring(2, 15);
        mockAuthState = { email, id: userId };
        localStorage.setItem(mockStorageKey, JSON.stringify(mockAuthState));
        localStorage.setItem('isAuthenticated', 'true');
    ...
    from: (table) => {
      return {
        select: (columns) => {
          return {
            eq: (column, value) => {
              console.log(`[Mock] Selecting from ${table} where ${column} = ${value}`);
        ...
        insert: (data) => {
          ...
          return { data: [{ ...data[0], id: 'mock_' + Math.random().toString(36).substring(2, 15) }], error: null };
        },
        update: (data) => {
          return {
            eq: (column, value) => {
