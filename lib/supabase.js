...
function getSupabaseCredentials() {
  ...
  if (typeof document !== 'undefined') {
    ...
    if (metaUrl && metaKey) {
      ...
      const url = metaUrl.replace(/<%=\s*process\.env\.SUPABASE_URL\s*%>/, '');
      const key = metaKey.replace(/<%=\s*process\.env\.SUPABASE_ANON_KEY\s*%>/, '');
  ...
  // If we can't get credentials, throw a descriptive error
