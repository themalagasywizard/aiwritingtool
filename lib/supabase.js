...
async function initSupabase() {
  if (!supabase) {
    try {
      ...
      // Validate URL and create client
      try {
        new URL(supabaseUrl); // This will throw if URL is invalid
        
        // Check if the global Supabase client is available
        if (typeof window.supabase === 'undefined') {
          console.error('Supabase client not loaded from script tag. Trying to use direct API access.');
          
          // In production, this is an error because we need the client
          if (window.location.hostname !== 'localhost' && 
              window.location.hostname !== '127.0.0.1') {
            // Try to load it dynamically before giving up
            try {
              // For production, always try to use the real client
              ...
              console.error('Failed to load Supabase dynamically:', dynamicImportError);
              throw new Error('Supabase client failed to load and we are in production mode.');
            }
          } else {
            // Only fall back to mock in development
            console.warn('Development mode: Using mock client as fallback');
        ...
        }, 3, 1000); // More retries with longer delay
        ...
        if (window.location.hostname === 'localhost' || 
            window.location.hostname === '127.0.0.1') {
          console.warn('Development mode: Falling back to mock client');
          return createMockClient();
        }
        
        // In production, try one more approach before failing
        try {
          // Try a different CDN as last resort
          const { createClient } = await import('https://unpkg.com/@supabase/supabase-js@2.39.3/dist/module/index.js');
          supabase = createClient(supabaseUrl, supabaseAnonKey);
          
          // Quick test
          const { error: finalTestError } = await supabase.auth.getSession();
          if (finalTestError) throw finalTestError;
          
          console.warn('Using backup Supabase client import method');
          return supabase;
        } catch (finalError) {
          console.error('All attempts to initialize Supabase failed:', finalError);
          throw new Error(`Cannot initialize Supabase client in production: ${urlError.message}`);
        }
      }
    } catch (error) {
      ...
      if (window.location.hostname === 'localhost' || 
          window.location.hostname === '127.0.0.1') {
        console.warn('Development mode: Using mock client due to initialization error');
        return createMockClient();
      }
      
      // In production, try one last approach with a simple basic client
      try {
        const { supabaseUrl, supabaseAnonKey } = getSupabaseCredentials();
        const simpleClient = {
          auth: {
          ...
          from: (table) => ({
            select: () => ({
              eq: () => ({
                order: () => ({
                  then: (resolve) => resolve({ data: [], error: new Error('Database unavailable') })
                })
              })
            })
          })
        ...
        console.error('Using emergency minimal client that will return empty results');
        return simpleClient;
      } catch (finalError) {
        // No more fallbacks, just throw
        throw error;
      }
    }
  }
