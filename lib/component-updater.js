...
/**
 * Process all buttons in the application to use the Shadcn Button component
 */
function processButtons() {
    // Get all buttons except those with data-no-shadcn attribute
    const buttons = document.querySelectorAll('button:not([data-no-shadcn])');
    
    buttons.forEach(button => {
        // Skip if already processed
        if (button.getAttribute('data-shadcn') === 'button') return;
        
        // Get current classes to determine variant
        const classes = button.className;
        let variant = 'default';
        
        // Skip AI mode buttons as they have custom styling
        if (button.classList.contains('ai-mode-btn')) {
            button.setAttribute('data-shadcn', 'button');
            button.setAttribute('data-variant', 'ai-mode');
            return;
        }
        
        if (classes.includes('secondary') || classes.includes('btn-secondary')) {
            variant = 'secondary';
        } else if (classes.includes('outline') || classes.includes('btn-outline')) {
            variant = 'outline';
        } else if (classes.includes('ghost') || classes.includes('btn-ghost')) {
            variant = 'ghost';
        ...
        // Add to the class for consistency
